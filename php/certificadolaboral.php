...
//Imagenes parametrizar

//$logo="TAL_TEX";
//$firma=$codigo;//Nunca cambiara esta imagen porque por politica el nombre de la imagen es la misma cedula del empleado o codigo 
//$sello="SELLO2";
...
//Datos del Empleado
$qry2="select *, convert(varchar(20),fec_ult_nom,105) as fec_ult_nom, convert(varchar(20),fec_ing,103) as fec_ing, convert(varchar(20),ini_cto,111) as ini_cto, convert(varchar(20),fec_ret,101) as fec_ret from empleados_basic a, cargos b where a.cod_car=b.cod_car and a.cod_epl='".$codigo."'";
...
$fecha_ing=$row2["fec_ing"];
$fecha_retiro=$row2["fec_ret"];
$fecha_anio_actual=$row2["fec_ult_nom"];
$sueldo=$row2["sal_bas"];
$cargo=$row2["nom_car"];
...
$nombrefirma=UTF8_encode($row3["nom_epl"]);
$apellidofirma=UTF8_encode($row3["ape_epl"]);
...
$anio_fecha_ing=substr($fecha_ing, 6, 4);
//var_dump($anio_fecha_ing);die("");


//var_dump($fecha_anio_actual);die("");

$anio_actual=substr($fecha_anio_actual, 6, 4);
$anio=substr($fecha_anio_actual, 6, 4);




if($anio_actual==$anio_fecha_ing){
	...
	//Identificar dia 30 o 15 de $fecha_ultima
	$dia=substr($fecha_ultima, 0, 2);
	//var_dump($dia);die("");

	//Identificar mes para dividirlo
	$mes=substr($fecha_ultima, 3, 2);
	$mes_fecha_ing=substr($fecha_ing, 3, 2);

	//Meses para promediar segun su ingreso
	$mes_tot=$mes-$mes_fecha_ing+1;

	//var_dump($mes_fecha_ing);die("");
	//var_dump($mes_tot);die("");
	...
	while($row0_2 = $rh0_2->FetchRow()){
		...
		if($fecha_ultima==$row0_2["fec_fin"]){
				...
				if($dia=="15"){
						...
						@$codigo_per_final=$codigo_periodo;
					 }
				//var_dump($codigo_periodo);die("");
				//19 octubre primera quincena
		}	
	}


	if($fecha_ultima=="15-01-".$anio_actual){
			
			$t_dias=1;

	}else{
			if($dia=="15" and $fecha_ultima!="15-01-".$anio_actual){
		
					$t_dias=30*$mes_tot-30;	
					//var_dump($t_dias);die("");
			}else{		
					$t_dias=30*$mes_tot;
					//var_dump($t_dias);die("entro");
				 }
		}
	...
	$qry0="select sum(h.valor) as valor
			...
			and h.ano = ".$anio_actual." and h.cod_per between 1 and '".$codigo_per_final."'";
	...
	$letrasnum1=numtoletras($promedio);


	
//FIN CASO 15-01-(anio-actual) QUE ENTRARON EN ESTE AÑO
}else{
//INICIO CASO "NORMAL" LOS QUE NO HAYAN INGRESADO A LA EMPRESA EN ANIO ACTUAL CORRESPONDIENTE

		
	//die("entro salario integral 31254005");
...
//Determino si el empleado que viene es de salario integral o no
$qry_int="select * from epl_grupos where cod_gru = '2'and
cod_epl ='".$codigo."'";

				  
$rhint = $conn->Execute($qry_int); 

$rowint = $rhint->FetchRow();


$registro=$rowint["cod_epl"];
...
if($porcentaje==NULL){//Integral (Pero sin compensacion) agalarza@imbanaco.com.co

	$qry_como="select h.cod_con, c.nom_con, sum(h.valor)as valor_como 
       from historia_liq h, conceptos c
       where h.cod_epl = '".$codigo."' and
    ( h.cod_con in (select cg.cod_con_base
         from conceptos_grupo cg,epl_grupos e
         where cg.cod_gru=e.cod_gru and e.cod_epl=h.cod_epl
         and cg.cod_liq = 01 and  cg.cod_con = 18 AND cg.pro_sn = 'S' )
or   h.cod_con in (select cod_sal_int from parametros))

       and h.cod_con not in(225) 
       and h.ano =".$anio." and c.cod_con = h.cod_con 
...
}else{ //Integral (Pero con compensacion)
...
$qrycer2="select porcentaje  from com_flexible_c where cod_epl = '".$codigo."' and estado='A'";

				  
$rhcer2 = $conn->Execute($qrycer2); 

$rowcer2 = $rhcer2->FetchRow();

$prom_flexible=0;

$porcentaje2=$rowcer2["porcentaje"];

if($porcentaje2==TRUE and $flag1=="false"){ //Javier Trejo
...
//var_dump($porcentaje);die("");
	...
	//var_dump($prom_flexible);die("entro");
...
//var_dump($porcentaje2);die("");
//Me genera el valor real del ingreso mensual con la compensacion flexible

if(empty($porcentaje)){//No Integral

$comp_flexi = $sueldo/(1-($porcentaje2/100));
...
if($prom_flexible>$comp_flexi){
	
							@$promedio_comp=$prom_flexible;
	
				}else{
							@$promedio_comp=$comp_flexi;
					 }

				$letrasnum14=numtoletras($promedio_comp);
...
<tr>
				  ...
				  $content.='<tr>
                    ...
                    <tr>
                        <td style="font-size:17px; text-align:justify">Salario Promedio: <span>'.$letrasnum1.'</span> ($<span>'.number_format($promedio, 0, ",", ".").'</span>) </td>
                    </tr>					
                    <tr>
                        <td><br></td>
                    </tr>
                 ...
                 $content.='<tr>
                    ...
                    <tr>
                        <td style="font-size:17px; text-align:justify">Ingreso Promedio, por concepto de tiempo extra, recargos nocturnos, dominicales, y/o festivos de: <span>'.$letrasnum14.'</span> ($<span>'.number_format($promedio_comp, 0, ",", ".").'</span>) </td>
                    </tr>					
                    <tr>
                        <td><br></td>
                    </tr>
