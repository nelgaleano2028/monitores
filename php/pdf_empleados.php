...
// $mail = new PJmail(); 
 	...
 	// $mail->text = "Se le hace entrega de su respectivo Comprobante de Pago."; 
 	// $mail->addbinattachement("comprobante_pago_".$com->fecha_comprobante().".pdf", $content_PDF); 
        // $mail->sendmail();
      
    //$email = "crifemarca@gmail.com, cristian.martinez@talentsw.com";
	...
	//echo $catidadCorreo = count($cedulas);
	...
	$content_PDF=$pdf->Output('', 'S');
       ...
       $enviado = $mail->AddAddress(@$emailUno[0]); // Esta es la dirección a donde enviamos @$_SESSION['cor']
       $mail->IsHTML(true); // El correo se envía como HTML
       $mail->Subject = "Envio de Comprobante"; // Este es el titulo del email.
       $empleado=@$emailUno[0];
       $body = "Señor(a) ".$emailUno[0]." su comprobante es:<br><br><br>";
       $mail->Body = "Se le hace entrega de su respectivo Comprobante de Pago."; // Mensaje a enviar
       $reset = $mail->AddStringAttachment($content_PDF,"comprobante.pdf"); //me permite enviar archivos adjuntos sin necesidad de indicar ruta 
