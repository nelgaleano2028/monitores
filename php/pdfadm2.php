...
for($i=0;$i<count($cedulas);$i++){
	...
	$com=new comprobante();
	$email=$com->email_empleado($cedulas[$i]).",";

	$emailUno = explode(",", $email);
	
	$varTEM = "";

if($emailUno[0] != $varTEM){
...
$qry0="select * from empleados_gral g, empleados_basic b where g.cod_epl=b.cod_epl and cedula='".$cedulas[$i]."'";
...
$qry2="SELECT e.cedula ced, e.ape_epl ape,
 ...
 WHERE  h.cod_epl between '".$cedulas[$i]."' and '".$cedulas[$i]."' 
...
//$fecha_actual=date("Y  m  d"); fecha actuak es la de expedicion.
...
$qry4="select *, convert(varchar(20),ini_cto,102) as StartTime, convert(varchar(20),fec_ret,102) as EndTime  from empleados_basic where cedula='".$cedulas[$i]."' and estado='A'";
...
//Funcion Cortar en la parte superior
...
$qry5="select tipo_doc from empleados_basic where cedula='".$cedulas[$i]."'";
...
if(isset($_POST['ver'])){
	...
	if($_POST['envio']){
		...
		$content_PDF=$html2pdf->Output('',true);
 		...
 		// $mail->addsubject("Certificado de Ingresos y Retenciones"); 
 		// $mail->text = "Se le hace entrega de su Certificado de Ingresos y Retenciones"; 
 		// $mail->addbinattachement("certificado.pdf", $content_PDF); 
 		// $res = $mail->sendmail(); 
		
		
		  //Estas dos líneas, cumplirían la función de encabezado (En mail() usado de esta forma: “From: Nombre <correo@dominio.com>”) de //correo.
       $mail->AddAddress(@$emailUno[0]); // Esta es la dirección a donde enviamos @$_SESSION['cor']
       $mail->IsHTML(true); // El correo se envía como HTML
       $mail->Subject = "Envio de Certificados de Ingresos y Retenciones"; // Este es el titulo del email.
       $empleado=@$emailUno[0];
       $body = "Señor(a) ".$emailUno[0]." Certificados de Ingresos y Retenciones es:<br><br><br>";
...
//Implode y explode codigo para hacer salto de linea de una variable
